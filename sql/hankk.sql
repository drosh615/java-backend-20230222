...
-- 회원가입 
insert into user (email, password, nickname, tell_number, address, address_detail, consent_personal_information)
values ('qwer@naver.com', sha2('1234',512), '010-1324-1234', 'det', '부산광역시', '부산진구', true);

-- 로그인 
-- 패스워드 암호화가 되어있지 않을 때 
select * from user
where email = 'qwer@naver.com'
and password = '1234';
-- 패스워드가 암호화 되어 있을 때 
select * from user
where email = 'qwer@naver.com';
...
values ('test title', 'test content', null, now(), 'qwer@naver.com');
...
-- 내 게시물 리스트
SELECT * FROM Board_View
WHERE writer_email = 'qwer@naver.com';

-- 좋아요
INSERT INTO liky (user_email, board_number)
VALUES ('qwer@naver.com', 1);

-- 좋아요 취소
DELETE FROM Liky
WHERE user_email = 'qwer@naver.com'
AND board_number = 1;

-- 댓글 작성
INSERT INTO Comment (user_email, board_number, comment_content, write_datetime)
VALUES ('qwer@naver.com', 1, 'Test Comment', now());
