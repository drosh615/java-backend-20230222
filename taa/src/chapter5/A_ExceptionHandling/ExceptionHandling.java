 ...
 * 
 * 시스템 에러 : 개발자가 코드상으로 조취를 취할 수 없는 문제 (전원 부족, 메모리 부족 등)
 * 컴파일 에러 : 컴파일이 불가능한 상태 (오타, 참조 타입 불일치 등)
 * 런타임 에러 : 프로그램이 실행 도중 발생하는 문제 (빌드 실패, 포트 중복, 데이터베이스 연결 실패 등)
 * 로직 에러 : 프로그램의 아웃풋이 예상과 다르게 나오는 문제
 * 예외 : 정상적인 입력에서는 정상 처리가 되지만 비정상적인 입력에서는 처리 되지 않는 문제 (널 값 참조, 로직 실행중 데이터베이스 연결 실패)
 */
...
//		System.out.println(numbers[4]);
		...
		/*
		 *  예외 처리
		 *  예외를 예상하고 그 상화에 대해서 적절한 대처를 해주는 것
		 ...
		 *  사용방법
		 *  try {예외가 발생할 수 있는 문장}
		 *  catch (예외타입 예외참조변수) {예외가 발생했을 때 문장}
		 */
		Scanner scanner = null;
		try {
